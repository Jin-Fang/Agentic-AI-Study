...
互补模式是 *agentic memory*：让 agent 定期把笔记写到磁盘，之后再加载。Anthropic 用 Claude 玩宝可梦作为例子：在数千个游戏步骤中，agent 维护统计、绘制地区地图、记录战斗策略，使它能在上下文重置后继续多小时训练序列 ([Anthropic - Effective Context Engineering for AI Agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents))。
