...
**过程可观测性**记录为什么当前工作应该被接受：任务范围、sprint contract、rubric、验证证据、排除项和 handoff note。Anthropic 的 generator-evaluator harness 把这一点显式化：实现前先协商 sprint contract，sprint 失败时 evaluator 按 rubric 给出具体反馈 ([Anthropic - Harness Design for Long-Running Application Development](https://www.anthropic.com/engineering/harness-design-long-running-apps))。
...
1. 捕获失败或异常的生产轨迹。
2. 脱敏敏感数据，并冻结相关环境或 fixture。
3. 抽取用户意图、工具序列、中间状态和最终 outcome。
4. 为修正后的行为写确定性断言或 model-graded assertion。
5. 把 case 加入 regression suite，并保留原始 trace 作为证据。
...
OpenAI 的 tax-agent 案例把这条回路又向前闭合了一步。专家纠正和生产 trace 不会直接改写已经部署的 agent；它们先变成经过审查的 finding、定制 eval，以及可在发布前验证的有界 Codex task ([OpenAI - Building Self-Improving Tax Agents with Codex](https://openai.com/index/building-self-improving-tax-agents-with-codex/))。

一条安全的改进 pipeline 在每一步都有明确 custody：

1. 捕获实践者纠正，以及促成该纠正的 trace 与 outcome。
2. 脱敏敏感数据，审查 finding，把重复项归并成稳定 failure class。
3. 用不可变证据把 failure class 转成可复现 regression eval。
4. 给 coding agent 一个有界 change task，而不是让它自由修改自己。
5. 用目标 eval、更广 regression suite、security check、成本与延迟共同 gate patch。
6. 在生产中 canary 新版本；监控原始失败信号；若变差则 rollback。

这是**受控自我改进（controlled self-improvement）**，不是不受控的 self-modification。已部署 agent 不会原地编辑自己的 prompt、tool、memory policy 或 evaluator。外部改进 loop 提出版本化变更；独立证据与 release control 决定它是否晋级。Evaluator integrity（第 10.4 节）会在两处生效：finding 必须真实，gate 也不能带着“应该 ship 这次修复”的偏见。

### 12.5 压力测试 load-bearing 组件
...
这不应被理解成“不要写仓库指令”。意思是根指令文件应该是路由器，而不是百科全书。OpenAI 的 Codex harness 指南也是同样形状：关键上下文应保留在仓库里，`AGENTS.md` 保持简短，指向更深入的文档，并尽可能用机械检查强制最重要的规则 ([OpenAI - Harness Engineering](https://openai.com/index/harness-engineering/))。
...
务实版本不是完全自动化架构搜索，而是有纪律的实验：

- 用同一 task suite A/B test 工具 schema 或 prompt 改动。
- 消融某个组件，例如 planner、context reset、memory layer 或 evaluator。
- 调整 cost controls、retry policy 和 tool-response truncation，观察质量在哪里下降。
- 衡量完整闭环，而不只是模型输出：success rate、pass^k reliability、latency、cost、human escalations 和 security false positives。
...
- 工具选择错误可能来自 **Tool** 层：动作空间太大，或 schema 没有暴露关键 affordance。
- 过早宣称完成可能来自 **Lifecycle** 层：退出条件没有被表示成持久状态机。
- 成本调优后的回归可能来自 **Observability** 与 **Execution** 层：资源限制改变了延迟、timeout 或 benchmark fidelity。
- 安全失败可能来自 **Governance** 层：训练时 alignment、部署配置和运行时 enforcement 之间的 policy、permission、audit hook 不一致。

实际做法是：在 trace review 中先标注层级假设，再改 prompt。好的 harness 迭代会先问：是哪一层让错误行为变得容易、不可见或便宜？然后再决定修复是 prompt、工具重设计、沙箱变更、新指标、更强 grader，还是 governance hook。
...
- **自我改进必须是外部、有界的 release loop**：审查并归并纠正，把它们转成 eval 与 scoped change task，再对版本化产物做 gate、canary 和 rollback。
...
- **臃肿的 AGENTS.md ROI 有限**：保持简短、人写，再通过渐进披露进入 repo-local 文档和机械检查。
- **可复用 harness 包保留来之不易的实践**：当 traces 证明某些模式有效，就把它们提升为 skills、模板、脚手架和检查。
- **Meta-harness 把 eval 变成设计搜索**：prompts、tools、retries、context policies 和 evaluators 都可像系统组件一样消融和优化。
- **层级归因避免只改 prompt**：先用 ETCLOVG 判断是哪一层让失败成为可能，再改指令。
