...
| [第 3 章](./03-compaction-memory-subagent.md) | 压缩、记忆与子代理模式 | 压缩、结构化笔记、反复复述、上下文防火墙、记忆架构、记忆投毒与信任升级、多 agent 失败分类 |
| [第 4 章](./04-tools-agent-computer-interface.md) | 工具与 Agent-Computer Interface | 工具设计、MCP 与私有连接、程序化工具调用、A2A、命名空间、token 高效响应、代码执行作为元工具 |
| [第 5 章](./05-sandboxing-guardrails.md) | 沙箱、护栏与安全自治 | 威胁模型、containment 模式、权限疲劳、agentic readiness、身份与治理、文件系统/网络隔离、hooks、运营安全 |
...
| [第 10 章](./10-evaluation.md) | 评估 | Eval 结构、grader 类型、evaluator integrity、pass@k 与 pass^k、readiness validation、八步路线图 |
| [第 11 章](./11-infrastructure-noise.md) | 基础设施噪声 | 资源配置对 benchmark 分数的影响 |
| [第 12 章](./12-trace-driven-iteration.md) | 基于 Trace 的迭代与 Model-Harness 共同演化 | Trace 反馈回路、span telemetry、regression extraction、受控自我改进、meta-harness、模型与 harness 的耦合 |
...
| [第 17 章](./17-cost-privacy-and-operations.md) | AgentOps——成本、隐私与生产运维 | 生命周期运维、预算、成本归因、语义缓存、数据治理、多租户、发布、监控与治理框架 |
| [第 18 章](./18-agent-fleets-identity-control-plane.md) | Agent Fleet、身份与控制平面 | Agent identity 与 registry、delegated authorization、gateway 与 policy enforcement、fleet lifecycle、lineage、audit 与 non-repudiation |
| [第 19 章](./19-outlook.md) | 展望 | 开放问题、跨层权衡与长期原则 |
