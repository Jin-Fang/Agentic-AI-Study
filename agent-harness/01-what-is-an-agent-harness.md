...
The agent loop — and the principle that a tool call is structured output the harness executes, not direct agency by the model — is established in the companion volume (see Foundations ch 1 and ch 12). As a quick recap: a raw model call is one-shot (text in, text out); an *agent* wraps it in a loop that assembles context, lets the model emit either a final answer or a *tool call* (structured output, typically JSON, naming a tool and arguments), has deterministic harness code execute that call, appends the observation, and repeats on the now-longer context. HumanLayer states the same point as "tools are just structured outputs" ([HumanLayer — 12-Factor Agents](https://www.humanlayer.dev/blog/12-factor-agents)): file edits, shell commands, browser clicks, and database writes become real only when the harness accepts the model's request and runs it. The model at the centre of this loop is what Anthropic calls the *augmented LLM* and the research literature calls *ReAct* (reason + act) (mechanics in Foundations ch 12).
