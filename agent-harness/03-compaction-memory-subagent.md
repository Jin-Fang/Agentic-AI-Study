...
- **Compaction extends task horizons**: summarize nearing-limit conversations, preserve key decisions, discard redundant tool outputs.
...
- **Leave errors in context**: self-healing only works when error traces are visible — removing them destroys the corrective signal.
