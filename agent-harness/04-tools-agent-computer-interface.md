...
This boundary view clarifies why MCP, A2A, OpenAPI, function calling, and AGENTS.md should not be treated as direct substitutes. They solve different integration problems. The harness designer's job is to decide which boundary is actually in play, then choose a protocol and governance model that preserves provenance, permissions, costs, and failure evidence across that boundary.
