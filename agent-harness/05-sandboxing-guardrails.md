...
The combination practitioners worry about most is sometimes called the *lethal trifecta* (a framing popularized by security researcher Simon Willison): access to private data, exposure to untrusted content, and the ability to communicate externally. Any one alone is survivable; all three in a single agent mean one injected instruction can read a secret and send it out. Most controls in this chapter work by breaking one leg of the trifecta — network isolation removes external communication, filesystem isolation removes private-data access, and approval gates put a human in the path of consequential actions.
