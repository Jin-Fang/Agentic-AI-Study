...
Retrieval 在运行时给模型补充外部信息。Karpathy 把 retrieval-augmented generation 介绍为一种方式：与其只依赖模型参数，不如把相关文档带进上下文 ([Intro to LLMs, around 00:41:33](https://www.youtube.com/watch?v=zjkBMFhNj_g&t=2493s))。RAG 的基础论文把参数化 seq2seq 模型和通过检索访问的非参数记忆结合起来 ([Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](https://arxiv.org/abs/2005.11401))。

对 harness engineer 来说，RAG 不是模型特性，而是 harness pattern。

## Embeddings

Embedding model 把文本映射到向量空间，使语义相关文本倾向于彼此接近。检索系统会嵌入文档或 chunk，再嵌入 query，并寻找相近向量。当 keyword search 漏掉改写或概念匹配时，embedding 很有用。

Embedding 不是魔法。它可能漏掉精确约束，混淆近邻，或检索到主题相似但实际无关的文本。Hybrid search、metadata filter、reranking 和领域 chunking 通常都很重要。
...
参数化记忆快而广，但过期且难审计。检索记忆慢且依赖基础设施，但可以是当前的、私有的、权限化的、可引用的。
...
每一步都可能失败。RAG 质量不只是模型质量。

## Chunking

文档通常太大，不能整体检索。它们会被切成 chunks。Chunking 决定模型看到什么 evidence。
...
好 chunk 保留语义单元：章节、段落、API 条目、ticket、代码符号或 policy clause。
...
高 recall 会检索更多可能相关材料。高 precision 检索更少但更干净的材料。LLM context 让这个取舍变得痛苦，因为额外文本不是免费的。无关检索文本会干扰模型或引入错误替代说法。

Reranking 有帮助。第一阶段 retriever 可以收集候选，reranker 再根据 query 更仔细打分。Harness 也可以让模型检查候选，但这会消耗 token，应该评估。
...
把所有可能来源都倒进 context，通常不如给模型正确工具。

## Context Pollution

RAG 可以减少某些幻觉，也可以制造幻觉。如果无关或不可信文本进入 context，模型可能因为它在场而把它当 evidence。检索文档可能过期、恶意、重复，或和 policy 矛盾。
